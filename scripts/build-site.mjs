...
const appStoreUrl = "https://apps.apple.com/app/id6608970522";
...
function pagePath(locale, pageKey, extension = false) {
  const prefix = locale.folder ? `/${locale.folder}` : "";
  if (pageKey === "home") return extension ? `${prefix}/index.html` : `${prefix}/`;
  return `${prefix}/${pages[pageKey].slug}${extension ? ".html" : ""}`;
}
...
function languageOptions(currentLocale, pageKey) {
  return locales.map((locale) => {
    ...
    return `<option value="${pagePath(locale, pageKey, true)}"${selected}>${escapeHtml(locale.label)}</option>`;
  }).join("");
}
...
function header(locale, pageKey) {
  ...
  const nav = ["home", "help", "support", "privacy"].map((key) => {
    ...
    return `<a href="${pagePath(locale, key, true)}"${current}>${escapeHtml(t[key])}</a>`;
  }).join("");
  return `<header class="topbar">
  <a class="brand" href="${pagePath(locale, "home", true)}" aria-label="Infinity Maze ${escapeHtml(t.home)}">
...
function storeButton(locale, compact = false) {
  const t = ui[locale.key];
  return `<a class="button primary${compact ? " compact" : ""}" href="${appStoreUrl}" aria-label="${escapeHtml(t.storeAria)}"><img class="store-icon" src="/images/app-store-icon.svg" alt="" width="24" height="24"><span>${escapeHtml(t.download)}</span></a>`;
}
...
<main>
  <section class="hero">
    <div class="hero-copy">
      ...
      <div class="actions">${storeButton(locale)}<a class="button secondary" href="${pagePath(locale, "help", true)}">${escapeHtml(t.help)}</a></div>
...
<main>
  ...
  <section class="support-grid">
    ...
    <article class="support-card"><span class="feature-icon">↗</span><h2>${escapeHtml(t.resourcesTitle)}</h2><div class="resource-links"><a href="${pagePath(locale, "help", true)}">${escapeHtml(t.help)}</a><a href="${pagePath(locale, "privacy", true)}">${escapeHtml(t.privacy)}</a>${storeButton(locale, true)}</div></article>
  </section>
</main>
${footer(locale)}
</div></div></body>
